...
namespace VMASharp {
    ...
    public sealed unsafe class VulkanMemoryAllocator : IDisposable {
        ...
        public VulkanMemoryAllocator(in VulkanMemoryAllocatorCreateInfo createInfo) {
            if (createInfo.VulkanAPIObject == null) throw new ArgumentNullException(nameof(createInfo.VulkanAPIObject), "API vtable is null");

            this.VkApi = createInfo.VulkanAPIObject;

            if (createInfo.Instance.Handle == default) throw new ArgumentNullException(nameof(createInfo.Instance));

            if (createInfo.LogicalDevice.Handle == default) throw new ArgumentNullException(nameof(createInfo.LogicalDevice));

            if (createInfo.PhysicalDevice.Handle == default) throw new ArgumentNullException(nameof(createInfo.PhysicalDevice));

            if (createInfo.VulkanAPIVersion < Vk.Version11) throw new NotSupportedException("Vulkan API Version of less than 1.1 is not supported");
        ...
        internal bool IsMemoryTypeNonCoherent(int memTypeIndex) {
            return (this.MemoryTypes[memTypeIndex].PropertyFlags & (MemoryPropertyFlags.MemoryPropertyHostVisibleBit | MemoryPropertyFlags.MemoryPropertyHostCoherentBit)) == MemoryPropertyFlags.MemoryPropertyHostVisibleBit;
        }

        internal long GetMemoryTypeMinAlignment(int memTypeIndex) {
            return this.IsMemoryTypeNonCoherent(memTypeIndex) ? (long)Math.Max(1, this.PhysicalDeviceProperties.Limits.NonCoherentAtomSize) : 1;
        }
