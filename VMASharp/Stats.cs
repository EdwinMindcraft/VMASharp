...
public struct StatInfo {
    ...
    internal static void Add(ref StatInfo info, in StatInfo other) {
        ...
        if (info.AllocationSizeMin > other.AllocationSizeMin) info.AllocationSizeMin = other.AllocationSizeMin;

        if (info.AllocationSizeMax < other.AllocationSizeMax) info.AllocationSizeMax = other.AllocationSizeMax;

        if (info.UnusedRangeSizeMin > other.UnusedRangeSizeMin) info.UnusedRangeSizeMin = other.UnusedRangeSizeMin;

        if (info.UnusedRangeSizeMax < other.UnusedRangeSizeMax) info.UnusedRangeSizeMax = other.UnusedRangeSizeMax;
    }
...
public class Stats {
    ...
    internal Stats(StatInfo[] memoryTypes, StatInfo[] memoryHeaps, in StatInfo total) {
        this.MemoryType = memoryTypes;
        this.MemoryHeap = memoryHeaps;
        this.Total = total;
    }
    ...
    internal Stats() {
        ...
        this.MemoryType = new StatInfo[Vk.MaxMemoryTypes];
        this.MemoryHeap = new StatInfo[Vk.MaxMemoryHeaps];

        for (int i = 0; i < Vk.MaxMemoryTypes; ++i)
            StatInfo.Init(out this.MemoryType[i]);

        for (int i = 0; i < Vk.MaxMemoryHeaps; ++i)
            StatInfo.Init(out this.MemoryHeap[i]);
    }
