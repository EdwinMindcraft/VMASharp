...
public sealed class DefragmentationContext : IDisposable {
    ...
    internal Result Defragment(
            ulong maxCPUBytesToMove, int maxCPUAllocationsToMove, ulong maxGPUBytesToMove,
            int maxGPUAllocationsToMove, CommandBuffer cbuffer, DefragmentationStats stats,
            DefragmentationFlags flags
        ) =>
        throw new NotImplementedException();

    internal Result DefragmentationPassBegin(ref DefragmentationPassMoveInfo[] Info) => throw new NotImplementedException();
