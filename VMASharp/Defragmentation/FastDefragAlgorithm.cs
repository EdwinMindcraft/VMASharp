...
internal sealed class FastDefragAlgorithm : DefragmentationAlgorithm {
    ...
    private class FreeSpaceDatabase {
        ...
        public FreeSpaceDatabase() {
            for (int i = 0; i < this.FreeSpaces.Length; ++i) this.FreeSpaces[i].BlockInfoIndex = -1;
        }

        public void Register(int blockInfoIndex, long offset, long size) {
            if (size < Helpers.MinFreeSuballocationSizeToRegister) return;

            int bestIndex = -1;
            for (int i = 0; i < this.FreeSpaces.Length; ++i) {
                ref FreeSpace space = ref this.FreeSpaces[i];
                ...
                if (space.Size < size && (bestIndex == -1 || space.Size < this.FreeSpaces[bestIndex].Size)) bestIndex = i;
            }

            if (bestIndex != -1) {
                ref FreeSpace bestSpace = ref this.FreeSpaces[bestIndex];
