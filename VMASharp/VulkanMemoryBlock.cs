...
internal class VulkanMemoryBlock : IDisposable {
    ...
    public void Dispose() {
        if (!this.MetaData.IsEmpty) throw new InvalidOperationException("Some allocations were not freed before destruction of this memory block!");

        Debug.Assert(this.DeviceMemory.Handle != 0);
    ...
    public void Validate() {
        Helpers.Validate(this.DeviceMemory.Handle != 0 && this.MetaData.Size > 0);

        this.MetaData.Validate();
    }

    public void CheckCorruption(VulkanMemoryAllocator allocator) {
        IntPtr data = this.Map(1);
    ...
    public unsafe IntPtr Map(int count) {
        ...
        lock (this.SyncLock) {
            ...
            if (this.mapCount > 0) {
                ...
                IntPtr pData;
                Result res = this.VkApi.MapMemory(this.Allocator.Device, this.DeviceMemory, 0, Vk.WholeSize, 0, (void**)&pData);

                if (res != Result.Success) throw new MapMemoryException(res);
    ...
    public void Unmap(int count) {
        ...
        lock (this.SyncLock) {
            ...
            if (newCount < 0) throw new InvalidOperationException("Memory block is being unmapped while it was not previously mapped");

            this.mapCount = newCount;

            if (newCount == 0) {
                this.MappedData = default;
                this.VkApi.UnmapMemory(this.Allocator.Device, this.DeviceMemory);
            }
        }
    }
